...
import { productsUtContent, utilitySubItems } from '../../../../content-bank/products-ut1';
...
export default function DetailsUt1() {
  ...
  const [currentProduct, setCurrentProduct] = useState(productsUtContent['Utility Variant']);
  const [activeThumbnail, setActiveThumbnail] = useState(productsUtContent['Utility Variant'].thumbnails[0]);
  ...
  useEffect(() => {
    ...
    if (product && productsUtContent[product]) {
      if (product === 'Utility Variant' && subItem) {
        const selectedSubItem = utilitySubItems.find(item => item.id === subItem);
        if (selectedSubItem) {
          setCurrentProduct({
            ...productsUtContent[product],
            name: selectedSubItem.name,
            thumbnails: selectedSubItem.thumbnails,
            specifications: [
              ...productsUtContent[product].specifications.filter(spec => 
                !['Lubang Efektif', 'Jarak Antar Reng', 'Sudut Atap'].includes(spec.label)
              ),
              ...selectedSubItem.specifications
            ],
            technicalSpecs: [
              ...productsUtContent[product].technicalSpecs.filter(tech => 
      ...
      setCurrentProduct(productsUtContent[product]);
      setActiveThumbnail(productsUtContent[product].thumbnails[0]);
    }
  }, []);

...
export default function ProductA() {
  ...
  useEffect(() => {
    ...
    // Check if there's an active subitem in sessionStorage
    const storedSubItem = sessionStorage.getItem('activeSubItem');
    if (storedSubItem) {
      // Check if it's a Victoria subproduct
  ...
  const handleMainItemClick = (item) => {
    ...
    if (item === 'Concrete Roof') {
      setShowSubmenu(showSubmenu && activeItem === 'Concrete Roof' ? false : true);
    } else if (item === 'Paving Block') {
      setShowSubmenu(showSubmenu && activeItem === 'Paving Block' ? false : true);
    } else {
  ...
  const productTypes = [
    ...
    { name: 'Victoria Pine', image: '/images/icon photo.png' }, 
    { name: 'Victoria Classic', image: '/images/icon photo.png' }
  ];
        ...
        <aside className="w-full lg:w-1/6 lg:sticky lg:top-[6.5rem] lg:h-[calc(100vh-6.5rem)] lg:overflow-y-auto">
          ...
          <ul className="space-y-2 text-sm">
            {mainProducts.map((item) => (
              <li key={item}>
                {item === 'Concrete Roof' || item === 'Paving Block' ? (
                  <>
                    <button
                      onClick={() => handleMainItemClick(item)}
                      className={`w-full text-left cursor-pointer font-medium px-2 ${
                        activeItem === item
                    ...
                    {showSubmenu && activeItem === item && (
                      <ul className="ml-4 mt-2 space-y-3 text-gray-600 text-xs border-l border-gray-300 pl-2 mb-4">
                        {(item === 'Concrete Roof' ? subProducts : subProductsPaving).map((sub) => (
                          <li key={sub}>
                            {sub === 'Victoria' && item === 'Concrete Roof' ? (
                              <>
                                <div 
                                ...
                                {showVictoriaSubmenu && (
                                  <ul className="ml-4 mt-1 space-y-2">
                                    {victoriaSubProducts.map((victoriaSub) => (
                                      <li key={victoriaSub}>
                                        <Link 
                                          href={`/produk/detail?product=Victoria ${victoriaSub}`}

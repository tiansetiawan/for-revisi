...
import { ConcreteRoofContent, concreteRoofProducts, PavingBlockContent, pavingBlockProducts, ConcreteBlockContent, concreteBlockProducts, UtilityContent, utilityProducts } from "../product-content";
              ...
              >
                {getCurrentProducts().map((product) => (
                  <div key={product.name} className="snap-start min-w-0 ps-12.5 group">
                    <Link href={`/produk/detail?product=${encodeURIComponent(product.name)}&category=${encodeURIComponent(activeItem)}`} className="block">
                      <div className="w-50 bg-gray-300 rounded-xl overflow-hidden shadow hover:shadow-lg transition flex flex-col items-center">
                        <div className="relative w-full h-45 flex items-center justify-center bg-white overflow-hidden">
                          <Image src={product.image} alt={product.name} fill className="object-scale-down transition-transform duration-500 group-hover:scale-105" sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" />
                        </div>
                        <div className="w-full text-center text-sm font-medium bg-[#E5ECF6] py-2 rounded-b-xl">{product.name}</div>
                      </div>
                    </Link>
                  </div>
                ))}
